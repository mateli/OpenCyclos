...
The groups are used to grant <a href="#permissions"><u>permissions</u></a> to
...
permissions, or to move users from one permission group to another.<br>
...
You can use the standard groups not only for granting permissions, but also for
...
by accident.
<br>
All of the groups come with default permissions, but it is possible to modify
these.
...
new administrators, setting permissions and changing the system configuration.
...
properties, modify permissions, and you can delete or add permission groups.
...
<ul>
	...
	<li><img border="0" src="${images}/permissions.gif" width="16" height="16">&nbsp;Selecting
	the permissions icon will open a page where you can set the permissions for the
	group.
	<li><img border="0" src="${images}/permissions_gray.gif" width="16" height="16">&nbsp;If
	the permission group icon is grey it means that no permissions can be set for
...
<br><br>Note that you cannot set permissions here; group properties and settings
are not the same as permissions. You can modify the group permissions by
clicking the <img border="0" src="${images}/permissions.gif" width="16"
	height="16"> permissions icon from the <a href="#manage_groups"><u>group
...
<ul>
	...
	<ul>
		...
		and members can perform any transaction (if they have the permissions, of
...
<ul>
	...
	not checked the user (and broker / admin depending on the permissions) can 
...
<ul>
	...
	group from an existing group via this drop down. Both settings and permissions
	are copied.
</ul>
...
<br>
<b>Important:</b>
After creating the new member group, you should set the permissions and
...
a case, you can still define different account settings and permissions for each
group.
...
the <a href="#permissions"><u>permissions</u></a> for the group, otherwise the
...
<br><br>
In cyclos, permissions are organized per group. For each group you can manage
...
administrator groups, where each administrator group has permissions to manage a
...
(member, admin, broker).
<br>
These basic permissions do not affect other functions, e.g. if a member cannot
...
In this window you can set the permissions for the system functions of an
administrator group. So this permissions window does not contain any permissions
...
<br><br>The permissions structure is quite straightforward. Most functions have
two permissions,&quot;View&quot; and &quot;Manage&quot;. <br>
...
<br>
The following permissions are available (you may want to use the links for more
information):
<ul>
	<li><b>Account Fees:</b> The view permissions allows the admin to go to the <a
		href="${pagePrefix}account_management#account_fee_overview"><u> Account Fee
	overview</u></a> page.<br>
	The &quot;Charge&quot; permissions allows to charge a &quot;manual&quot; or failed
	...
	<ul>
		...
		payments</u></a></b>
		View authorized payments menu item.
	...
	<ul>
		...
		groups in his <a href="#manage_groups"><u>group permissions window</u></a>.
	...
	The other permissions (process, check and manage) are operational permissions
	...
	permissions, but you can also choose &quot;manage customized files&quot;. This
	...
	the members. This permissions allows the admin to create/define such a member
	record type.<br>
	<br>
	...
	system.<br>
	<br>
	<li><b>System Payments:</b> Here you can assign permissions for payments
	...
	<ul>
		...
		<li><b>Chargeback payment:</b> An administrator with this permissions can
	...
	<ul>
		...
		view connected members of group he/she has view permissions for.
		...
		he/she has view permissions for.
...
In this window you can set the permissions for the member functions of an
administrator group. These permissions would typically apply to an &quot;Account
administrator&quot; group. The permissions structure is straightforward. Most
functions have &quot;View&quot; and &quot;Manage&quot; permissons and sometimes
extra specific permissions.
<br>
...
<br><br>The following permissions can be set - you may want to use the links for
more information:
<ul>
	<li><b>Access:</b> permissions to control the access by the members of the
	...
	<br>
	<br>
	<li><b>Accounts:</b> permissions concerning the permissions for the admin
	...
	<ul>
		...
		<li><b>View authorized payments:</b> Allows the admin to view <a
			href="${pagePrefix}payments#authorized"><u>View authorized payments</u></a>.
	...
	<li><b>Products and Services:</b> These permissions allow the admin of
	...
	<br>
	<br>
	<li><b>Guarantees:</b> Various permissions concerning the <a
	...
	<ul>
		...
		<li><b>View authorized loans:</b> For some loans special authorization is needed.
	...
	<br>
	<br>
	<li><b>Member Payments:</b> this is a set of permissions concerning the <a
	...
	This permissions allow to view or view & modify the transaction feedbacks by an admin.<br>
	...
	<br>
	<li><b>Members:</b> some miscellaneous permissions on what the admin can
...
In this window you can set the permissions of the admininstrator functions of an
...
receive the permissions you checked. It is common pratice to have a limited (if
not one) number of administrators with this kind of permission, because these
are top level permissions.
<br>
...
<br><br>If the function has specific permissions (apart from manage and view) the
...
<ul>
	...
	<br>
	<li><b>Administrators:</b> these are permissions concerning other admins,
...
<ul>
	...
	needs to have the permissions as well.
	<br>
	<br>
	...
	<ul>
		...
		<li><b>Authorize when payment receiver:</b> allows the member to <a
			href="${pagePrefix}payments#authorized"><u>Authorize</u></a> a payment if he
		is the receiver.
		<br>
		...
		<li><b>Cancel payment pending authorization:</b> When <a
			href="${pagePrefix}payments#authorized"><u> authorized payments</u></a>
		are used, this will allow members to cancel their authorized payments once
		they have been created, but not yet been authorized.
	...
	use a specific group to view you will need to set the same permissions for the
	view permissions in Ads.<br>
	<br>
...
These are typically the permissions about the
<a href="${pagePrefix}brokering"> <u>brokering</u></a>
functions a broker may perform. This means you can define the type of functions
brokers of this group can do in relation to their members. The permissions
...
<ul>
	...
	<ul>
		...
		the <a href="${pagePrefix}payments#authorized"><u>authorized
		payments</u></a> system.
		<li><b>Cancel authorized payment as member</b>
		<li><b>Cancel <a href="${pagePrefix}payments#scheduled"><u>scheduled
		payment</u></a> as member</b>.
