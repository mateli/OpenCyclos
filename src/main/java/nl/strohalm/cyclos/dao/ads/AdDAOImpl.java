...
//                searcher.close();
    ...
    public List<Ad> search(final AdQuery query) {
        ...
        jpaCustomFieldHandler.appendJoins(hql, "ad.customValues", query.getAdValues());
        jpaCustomFieldHandler.appendJoins(hql, "m.customValues", query.getMemberValues());
        JpaQueryHelper.appendJoinFetch(hql, getEntityType(), "ad", query.getFetch());
        ...
        JpaQueryHelper.addParameterToQuery(hql, namedParameters, "ad.category.active", true);
        JpaQueryHelper.addParameterToQuery(hql, namedParameters, "ad.id", query.getId());
        JpaQueryHelper.addParameterToQuery(hql, namedParameters, "ad.membersNotified", query.getMembersNotified());
        JpaQueryHelper.addParameterToQuery(hql, namedParameters, "ad.externalPublication", query.getExternalPublication());
        JpaQueryHelper.addParameterToQuery(hql, namedParameters, "m", query.getOwner());
        // Group filters are handled at service level
        JpaQueryHelper.addInParameterToQuery(hql, namedParameters, "m.group", query.getGroups());
        JpaQueryHelper.addParameterToQuery(hql, namedParameters, "ad.tradeType", query.getTradeType());
        JpaQueryHelper.addParameterToQueryOperator(hql, namedParameters, "ad.price", ">=", query.getInitialPrice());
        JpaQueryHelper.addParameterToQueryOperator(hql, namedParameters, "ad.price", "<=", query.getFinalPrice());
        JpaQueryHelper.addParameterToQuery(hql, namedParameters, "ad.currency", query.getCurrency());
        JpaQueryHelper.addPeriodParameterToQuery(hql, namedParameters, "ad.publicationPeriod.begin", Period.day(query.getBeginDate()));
        JpaQueryHelper.addPeriodParameterToQuery(hql, namedParameters, "ad.publicationPeriod.end", Period.day(query.getEndDate()));
        ...
        if (query.getSince() != null && query.getSince().getNumber() > 0) {
            ...
            JpaQueryHelper.addParameterToQueryOperator(hql, namedParameters, "ad.publicationPeriod.begin", ">=", since);
        }
        JpaQueryHelper.addPeriodParameterToQuery(hql, namedParameters, "ad.publicationPeriod.begin", query.getPeriod());
        ...
        JpaQueryHelper.addParameterToQueryOperator(hql, namedParameters, "ad.creationDate", "<=", historyDate);
        ...
        // Custom Values
        jpaCustomFieldHandler.appendConditions(hql, namedParameters, query.getAdValues());
        jpaCustomFieldHandler.appendConditions(hql, namedParameters, query.getMemberValues());

        // Handle order
        if (query.isRandomOrder()) {
            JpaQueryHelper.appendOrder(hql, "rand()");
        } else {
            JpaQueryHelper.appendOrder(hql, "ad.publicationPeriod.begin desc, ad.id desc");
        }
    ...
    private Integer count(Calendar date, final Collection<? extends Group> groups, final Ad.Status status, final String projection) {
        ...
        switch (status) {
            ...
            case PERMANENT:
                ...
                break;
            case EXPIRED:
                JpaQueryHelper.addParameterToQueryOperator(hql, namedParameters, "ad.publicationPeriod.begin", "<=", date);
                JpaQueryHelper.addParameterToQueryOperator(hql, namedParameters, "ad.publicationPeriod.end", "<=", date);
                break;
            case SCHEDULED:
                JpaQueryHelper.addParameterToQueryOperator(hql, namedParameters, "ad.publicationPeriod.begin", ">", date);
                break;
